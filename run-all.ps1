...
$bbsSignerToken = "bbs-signer-dev-token-2026"
...
Start-ServiceWindow -Title "SSI BBS Signer" -Command "cd /d `"$root\bbs-signer-service`" && set BBS_SIGNER_ALLOWED_ORIGINS=http://localhost:5173,http://127.0.0.1:5173 && set BBS_SIGNER_AUTH_TOKEN=$bbsSignerToken && go run ."
...
Start-ServiceWindow -Title "SSI Frontend" -Command "cd /d `"$root\SSI-Frontend`" && set VITE_BBS_SIGNER_AUTH_TOKEN=$bbsSignerToken && if not exist node_modules npm install && npm run dev"
